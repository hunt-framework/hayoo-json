[
  {
    "cmd": "update",
    "document": {
      "uri": "http://hackage.haskell.org/package/regex-compat/docs/Text-Regex.html#",
      "description": {
        "fct-descr": "\u003cdiv class=\"doc\"\u003e\u003cp\u003eRegular expression matching.  Uses the POSIX regular expression\n interface in \u003ca\u003eText.Regex.Posix\u003c/a\u003e.\n\u003c/p\u003e\u003c/div\u003e",
        "fct-module": "Text.Regex",
        "fct-package": "regex-compat",
        "fct-signature": "module",
        "fct-source": "src/Text-Regex.html",
        "fct-type": "module",
        "title": "Regex"
      },
      "index": {
        "description": "Regular expression matching Uses the POSIX regular expression interface in Text.Regex.Posix",
        "hierarchy": "Text Regex",
        "module": "Text.Regex",
        "name": "Regex",
        "normalized": "",
        "package": "regex-compat",
        "partial": "Regex",
        "signature": ""
      }
    }
  },
  {
    "cmd": "update",
    "document": {
      "uri": "http://hackage.haskell.org/package/regex-compat/docs/Text-Regex.html#t:Regex",
      "description": {
        "fct-descr": "\u003cp\u003eA compiled regular expression.\n\u003c/p\u003e",
        "fct-module": "Text.Regex",
        "fct-package": "regex-compat",
        "fct-signature": "data",
        "fct-type": "data",
        "title": "Regex"
      },
      "index": {
        "description": "compiled regular expression",
        "hierarchy": "Text Regex",
        "module": "Text.Regex",
        "name": "Regex",
        "normalized": "",
        "package": "regex-compat",
        "partial": "Regex",
        "signature": ""
      }
    }
  },
  {
    "cmd": "update",
    "document": {
      "uri": "http://hackage.haskell.org/package/regex-compat/docs/Text-Regex.html#v:matchRegex",
      "description": {
        "fct-descr": "\u003cp\u003eMatch a regular expression against a string\n\u003c/p\u003e",
        "fct-module": "Text.Regex",
        "fct-package": "regex-compat",
        "fct-signature": "Regex-\u003e String-\u003e Maybe [String]",
        "fct-type": "function",
        "title": "matchRegex"
      },
      "index": {
        "description": "Match regular expression against string",
        "hierarchy": "Text Regex",
        "module": "Text.Regex",
        "name": "matchRegex",
        "normalized": "Regex-\u003eString-\u003eMaybe[String]",
        "package": "regex-compat",
        "partial": "Regex",
        "signature": "Regex-\u003eString-\u003eMaybe[String]"
      }
    }
  },
  {
    "cmd": "update",
    "document": {
      "uri": "http://hackage.haskell.org/package/regex-compat/docs/Text-Regex.html#v:matchRegexAll",
      "description": {
        "fct-descr": "\u003cp\u003eMatch a regular expression against a string, returning more information\n about the match.\n\u003c/p\u003e",
        "fct-module": "Text.Regex",
        "fct-package": "regex-compat",
        "fct-signature": "Regex-\u003e String-\u003e Maybe (String, String, String, [String])",
        "fct-type": "function",
        "title": "matchRegexAll"
      },
      "index": {
        "description": "Match regular expression against string returning more information about the match",
        "hierarchy": "Text Regex",
        "module": "Text.Regex",
        "name": "matchRegexAll",
        "normalized": "Regex-\u003eString-\u003eMaybe(String,String,String,[String])",
        "package": "regex-compat",
        "partial": "Regex All",
        "signature": "Regex-\u003eString-\u003eMaybe(String,String,String,[String])"
      }
    }
  },
  {
    "cmd": "update",
    "document": {
      "uri": "http://hackage.haskell.org/package/regex-compat/docs/Text-Regex.html#v:mkRegex",
      "description": {
        "fct-descr": "\u003cp\u003eMakes a regular expression with the default options (multi-line,\n case-sensitive).  The syntax of regular expressions is\n otherwise that of \u003ccode\u003eegrep\u003c/code\u003e (i.e. POSIX \"extended\" regular\n expressions).\n\u003c/p\u003e",
        "fct-module": "Text.Regex",
        "fct-package": "regex-compat",
        "fct-signature": "String -\u003e Regex",
        "fct-source": "src/Text-Regex.html#mkRegex",
        "fct-type": "function",
        "title": "mkRegex"
      },
      "index": {
        "description": "Makes regular expression with the default options multi-line case-sensitive The syntax of regular expressions is otherwise that of egrep i.e POSIX extended regular expressions",
        "hierarchy": "Text Regex",
        "module": "Text.Regex",
        "name": "mkRegex",
        "normalized": "String-\u003eRegex",
        "package": "regex-compat",
        "partial": "Regex",
        "signature": "String-\u003eRegex"
      }
    }
  },
  {
    "cmd": "update",
    "document": {
      "uri": "http://hackage.haskell.org/package/regex-compat/docs/Text-Regex.html#v:mkRegexWithOpts",
      "description": {
        "fct-descr": "\u003cp\u003eMakes a regular expression, where the multi-line and\n case-sensitive options can be changed from the default settings.\n\u003c/p\u003e",
        "fct-module": "Text.Regex",
        "fct-package": "regex-compat",
        "fct-signature": "String-\u003e Bool-\u003e Bool-\u003e Regex",
        "fct-type": "function",
        "title": "mkRegexWithOpts"
      },
      "index": {
        "description": "Makes regular expression where the multi-line and case-sensitive options can be changed from the default settings",
        "hierarchy": "Text Regex",
        "module": "Text.Regex",
        "name": "mkRegexWithOpts",
        "normalized": "String-\u003eBool-\u003eBool-\u003eRegex",
        "package": "regex-compat",
        "partial": "Regex With Opts",
        "signature": "String-\u003eBool-\u003eBool-\u003eRegex"
      }
    }
  },
  {
    "cmd": "update",
    "document": {
      "uri": "http://hackage.haskell.org/package/regex-compat/docs/Text-Regex.html#v:splitRegex",
      "description": {
        "fct-descr": "\u003cp\u003eSplits a string based on a regular expression.  The regular expression\nshould identify one delimiter.\n\u003c/p\u003e\u003cp\u003eThis does not advance and produces an infinite list of [] if the regex\nmatches an empty string.  This misfeature is here to match the\nbehavior of the the original Text.Regex API.\n\u003c/p\u003e",
        "fct-module": "Text.Regex",
        "fct-package": "regex-compat",
        "fct-signature": "Regex -\u003e String -\u003e [String]",
        "fct-source": "src/Text-Regex.html#splitRegex",
        "fct-type": "function",
        "title": "splitRegex"
      },
      "index": {
        "description": "Splits string based on regular expression The regular expression should identify one delimiter This does not advance and produces an infinite list of if the regex matches an empty string This misfeature is here to match the behavior of the the original Text.Regex API",
        "hierarchy": "Text Regex",
        "module": "Text.Regex",
        "name": "splitRegex",
        "normalized": "Regex-\u003eString-\u003e[String]",
        "package": "regex-compat",
        "partial": "Regex",
        "signature": "Regex-\u003eString-\u003e[String]"
      }
    }
  },
  {
    "cmd": "update",
    "document": {
      "uri": "http://hackage.haskell.org/package/regex-compat/docs/Text-Regex.html#v:subRegex",
      "description": {
        "fct-descr": "\u003cp\u003eReplaces every occurance of the given regexp with the replacement string.\n\u003c/p\u003e\u003cp\u003eIn the replacement string, \u003ccode\u003e\"\\1\"\u003c/code\u003e refers to the first substring;\n\u003ccode\u003e\"\\2\"\u003c/code\u003e to the second, etc; and \u003ccode\u003e\"\\0\"\u003c/code\u003e to the entire match.\n\u003ccode\u003e\"\\\\\\\\\"\u003c/code\u003e will insert a literal backslash.\n\u003c/p\u003e\u003cp\u003eThis does not advance if the regex matches an empty string.  This\nmisfeature is here to match the behavior of the the original\nText.Regex API.\n\u003c/p\u003e",
        "fct-module": "Text.Regex",
        "fct-package": "regex-compat",
        "fct-signature": "Regex-\u003e String-\u003e String-\u003e String",
        "fct-type": "function",
        "title": "subRegex"
      },
      "index": {
        "description": "Replaces every occurance of the given regexp with the replacement string In the replacement string refers to the first substring to the second etc and to the entire match will insert literal backslash This does not advance if the regex matches an empty string This misfeature is here to match the behavior of the the original Text.Regex API",
        "hierarchy": "Text Regex",
        "module": "Text.Regex",
        "name": "subRegex",
        "normalized": "Regex-\u003eString-\u003eString-\u003eString",
        "package": "regex-compat",
        "partial": "Regex",
        "signature": "Regex-\u003eString-\u003eString-\u003eString"
      }
    }
  }
]